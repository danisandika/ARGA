...
$(document).ready(function() {
  $('#customer_id').on('input',function(){
    ...
    $.ajax({
      url: '<?php echo base_url('CSActivity/get_data_customer');?>',
      type: 'POST',
      dataType: 'JSON',
      data: {customer_id: customer_id},
      cache:false,
      success:function(data){
        $.each(data,function(customer_channel,customer_alamat) {
          $('[name="customer_alamat"]').val(data.customer_alamat)
          $('[name="customer_channel"]').val(data.customer_channel);
        });
      }
    });
...
<script type="text/javascript">
    $(document).ready(function() {
      ...
      $('.view_activity').click(function(){
          ...
          $.ajax({
              url : "<?php echo site_url('CSActivity/get_detail_revenue')?>",
              data:{id : id},
              method:'GET',
              dataType:'json',
              success:function(response) {
                $('[name="week1"]').val(formatter.format(response.rev_week1)); //hold the response in id and show on popup
                $('#week2').val(formatter.format(response.rev_week2));
                $('#week3').val(formatter.format(response.rev_week3));
                $('#week4').val(formatter.format(response.rev_week4));
                $('#totall').val(formatter.format(response.rev_total));
                //console.log(response.rev_total);
